...
import { CalendarEvent, CalendarAnalytics, CalendarAvailability, AvailabilitySlot } from '@/types/calendar';
...
export class GoogleCalendarClient {
  ...
  async generateAnalytics(dateRange?: { start: Date; end: Date }): Promise<CalendarAnalytics> {
    ...
    const topAttendees = Object.entries(attendeeCount)
      .map(([key, count]) => {
        ...
        return { email, displayName: displayName !== email ? displayName : undefined, meetingCount: count };
      })
      .sort((a, b) => b.meetingCount - a.meetingCount)
      .slice(0, 10);

    const totalMeetingHours = totalMeetingMinutes / 60;
    const averageMeetingLength = totalEvents > 0 ? totalMeetingMinutes / totalEvents : 0;

    // Get today's busy/free hours
    const todaysEvents = await this.getTodaysEvents();
    const { totalBusyTime } = this.calculateAvailability(new Date(), todaysEvents);
  ...
  async getAvailability(date: Date): Promise<CalendarAvailability> {
    ...
    const endOfDay = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 17, 0); // 5 PM
  ...
  private calculateAvailability(date: Date, events: CalendarEvent[]) {
    ...
    const endOfDay = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 17, 0);
    ...
    const totalBusyTime = busySlots.reduce((total, slot) => total + slot.duration, 0);
    const totalFreeTime = freeSlots.reduce((total, slot) => total + slot.duration, 0);
