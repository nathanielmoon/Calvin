...
import { calendarApiRateLimiter, createRateLimitResponse } from '@/lib/rate-limiter';
  ...
  try {
    ...
    if (days === 1) {
      ...
      return NextResponse.json({
        ...
        summary: {
          totalDays: days,
          averageFreeTime: availabilities.reduce((sum, day) => sum + day.totalFreeTime, 0) / days,
          averageBusyTime: availabilities.reduce((sum, day) => sum + day.totalBusyTime, 0) / days,
        },
        generated_at: new Date().toISOString(),
      });
    }
